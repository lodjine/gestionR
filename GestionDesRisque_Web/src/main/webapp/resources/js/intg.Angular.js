...
var myApp = angular.module('intgApp', []);
...
var TableCtrl = myApp.controller('TableCtrl', function ($scope, $filter, filteredListService) {
    ...
    $scope.ic = $scope.allItems[0].Iintg ;
    ...
    $scope.addImp = function () {
    	UpdateConfWithNewObeject($scope.ImpLabel,$scope.allItems[0].integId,$scope.ImpValue,"impl") ;
    	$scope.allItems = getDummyData();
   	 $scope.filteredList = $scope.allItems;
        $scope.pagination();
        $scope.resetAll();
...
function getDummyData() {
	...
	var xxx = "" ; 
	$.ajax({
	    url:'/GestionDesRisque_Web/Seekintg/'+confId+'/',
	    dataType:'json',
	    type:'get',
	    async:false,

...
public class ConfidentialiteController {
	...
	public ModelAndView showConf(HttpSession session){
	...
	@RequestMapping(value = "/updateConfWithOldObject/{label}/{confId}/{value}/{type}/{id}", method = RequestMethod.GET)
    public @ResponseBody int UpdateConf(@PathVariable("label") String label,@PathVariable("confId") int id,@PathVariable("value") int value , @PathVariable("type") String type,@PathVariable("id") int idObject,  HttpSession session) {
		Confidentialite conf = confidentialiteServiceImpl.getById(id);

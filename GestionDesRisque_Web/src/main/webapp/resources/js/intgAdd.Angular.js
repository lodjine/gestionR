...
var myApp = angular.module('intgAddApp', []);
...
var TableCtrl = myApp.controller('TableCtrl', function ($scope, $filter, filteredListService) {
    ...
    $scope.iConf = $scope.allItems[0].Iintg ;
    ...
    $scope.updatTotal = function(){
    	...
    	if($scope.iconfEq == 0){
    		...
    		UpdateConfWithNewObeject("nothing",$scope.allItems[0].integId,xx,"Resultat") ;
    	}
    }
    $scope.add = function () {
    	var i = 0 ; 
    		if($scope.allItems[0].integId == 0){
          i=  saveRisk($scope.risqueLabel,$scope.allItems[0].integId);
           }else{
            	 i=  saveRisk($scope.risqueLabel,$scope.allItems[0].integId);
           }
          $scope.filteredList = $scope.allItems;
    ...
    $scope.addMesure = function () {
    	...
    	if($scope.iconfEq == 0){
    		...
    		if($scope.MesureSelect.repeatSelect == null){
    			UpdateConfWithNewObeject($scope.MesureLabel,$scope.allItems[0].integId,$scope.MesureValue,"Mesure") ;
    		}else{
...
$scope.addVul = function () {
    	...
    	if($scope.iconfEq == 0){
    		...
    		if($scope.vulSelect.repeatSelect == null){
    			UpdateConfWithNewObeject($scope.VulLabel,$scope.allItems[0].integId,$scope.VulValue,"Vul") ;
    		}else{
...
$scope.updateconfLabel = function(){
    ...
    $scope.addImp = function () {
    	...
    	if($scope.iconfEq == 0){
    		...
    		if($scope.impSelect.repeatSelect == null){
    			UpdateConfWithNewObeject($scope.ImpLabel,$scope.allItems[0].integId,$scope.ImpValue,"imp") ;
    		}else{
    ...
    $scope.addRisk = function () {
    	...
    	if($scope.iconfEq == 0){
    		if($scope.RiskSelect.repeatSelect == null){
    			...
    			UpdateConfWithNewObeject($scope.RiskLabel,$scope.allItems[0].integId,$scope.ProcSelect.repeatSelect,"risk") ;
    	}
 ...
 $scope.updateMesure = function(){
	 	...
	 	if($scope.MesureSelect.repeatSelect == null){
	     	...
	     	$scope.total = ( parseInt($scope.vulnValue)* parseInt($scope.impactsValue) * $scope.allItems[0].Iintg)- parseInt($scope.mesureValue)   ;
    ...
    $scope.deleteMesure= function(index){
    	...
    	deleteObject($scope.allItems[0].mesures[index].mesureId,"Mesure",$scope.allItems[0].integId);
    ...
    $scope.deleteVul= function(index){
    	deleteObject($scope.allItems[0].mesures[index].vulnId,"Vul",$scope.allItems[0].integId);
...
function getDummyData(x) {
	...
	var xxx = "" ; 
	$.ajax({
	    url:'/GestionDesRisque_Web/Seekintg/'+x+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
...
function UpdateConfWithNewObeject(label,confId,value,type){
	var xd = 0 ;
	$.ajax({
	    url:'/GestionDesRisque_Web/updateintg/'+label+'/'+confId+'/'+value+'/'+type+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
	    success: function(data) {
...
function UpdateConfWithOldObject(confId,type,id){
	var xd = 0 ;
	$.ajax({
	    url:'/GestionDesRisque_Web/updateConfWithOldObjectintg/'+confId+'/'+type+'/'+id+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
	    success: function(data) {
...
function PersisteConfWithNewObeject(label,value,type){
	var xd = 0 ;
	$.ajax({
	    url:'/GestionDesRisque_Web/PersisteupdateConfintg/'+label+'/'+value+'/'+type+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
	    success: function(data) {
...
function PersisteConfWithOldObject(type,id,idProces){
	var xd = 0 ;
	$.ajax({
	    url:'/GestionDesRisque_Web/PersisteConfWithOldObjectintg/'+type+'/'+id+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
	    success: function(data) {
...
function deleteObject(id,type,confId){
	$.ajax({
	    url:'/GestionDesRisque_Web/deleteObjectintg/'+id+'/'+type+'/'+confId+'/',
	    dataType:'json',
	    type:'get',
	    async:false

...
var myApp = angular.module('dispAddApp', []);
...
var TableCtrl = myApp.controller('TableCtrl', function ($scope, $filter, filteredListService) {
    ...
    $scope.updatTotal = function(){
    	...
    	if($scope.iconfEq == 0){
    		...
    		UpdateConfWithNewObeject("nothing",$scope.allItems[0].DispId,xx,"Resultat") ;
    	}
    }
    $scope.add = function () {
    	var i = 0 ; 
...
$scope.addVul = function () {
    ...
    $scope.addImp = function () {
    	...
    	if($scope.iconfEq == 0){
    		...
    		if($scope.impSelect.repeatSelect == null){
    			UpdateConfWithNewObeject($scope.ImpLabel,$scope.allItems[0].DispId,$scope.ImpValue,"imp") ;
    		}else{
    ...
    $scope.addRisk = function () {
    	...
    	if($scope.iconfEq == 0){
    		if($scope.RiskSelect.repeatSelect == null){
    			...
    			UpdateConfWithNewObeject($scope.RiskLabel,$scope.allItems[0].DispId,$scope.ProcSelect.repeatSelect,"risk") ;
    	}
 ...
 $scope.updateMesure = function(){
	 	...
	 	if($scope.MesureSelect.repeatSelect == null){
	     	...
	     	$scope.total = ( parseInt($scope.vulnValue)* parseInt($scope.impactsValue) * $scope.allItems[0].iDisp)- parseInt($scope.mesureValue)   ;
...
function getDummyData(x) {
	...
	var xxx = "" ; 
	$.ajax({
	    url:'/GestionDesRisque_Web/SeekDisp/'+x+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
...
function PersisteConfWithNewObeject(label,value,type){
	var xd = 0 ;
	$.ajax({
	    url:'/GestionDesRisque_Web/PersisteupdateConfdisp/'+label+'/'+value+'/'+type+'/',
	    dataType:'json',
	    type:'get',
	    async:false,
	    success: function(data) {
